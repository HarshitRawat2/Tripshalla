  ...
  return (
    <section className="max-w-7xl mx-auto px-6 py-12 font-sans overflow-visible">
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-400 text-sm mt-1 font-medium">Price updated as of 21st Jan</p>
      </div>
      ...
      <div className="relative group">
        ...
        >
          {packages.map((pkg) => (
            <div 
              key={pkg.id} 
              className="min-w-[300px] md:min-w-[380px]  h-[520px] relative rounded-3xl overflow-hidden snap-start shadow-lg group/card"
            >
              ...
              <div className={`absolute inset-0 bg-gradient-to-t ${pkg.overlayColor} z-10`} />
              ...
              <div className="absolute inset-0 z-20 flex flex-col justify-end p-6 text-white">
                ...
                <div className="flex flex-wrap gap-2 mb-6">
                  {pkg.tags.map((tag, i) => (
                    <span key={i} className="text-[10px] font-medium bg-white/10 px-2 py-1 rounded backdrop-blur-sm border border-white/10">
                      {tag}
                    </span>
                  ))}
                </div>
                ...
                <div className="space-y-4">
                  <div className="flex items-end gap-2">
                    <span className="text-2xl font-black italic">INR {pkg.price}</span>
                    <span className="text-xs line-through opacity-60 mb-1">INR {pkg.oldPrice}</span>
                    <div className="relative mb-1">
                       <span className="text-[9px] bg-white text-black px-2 py-0.5 rounded-sm font-black">SAVE INR {pkg.save}</span>
                    </div>
                  </div>
